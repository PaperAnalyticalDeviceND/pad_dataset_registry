...
def get_dataset_directories(
    owner: str = "PaperAnalyticalDeviceND",
    repo: str = "pad_dataset_registry", 
...
def detect_csv_schema(csv_data: List[Dict[str, Any]]) -> Dict[str, Dict[str, str]]:
    """
    Detect the schema (column types) of a CSV file with improved type detection.
    
    Args:
        csv_data (List[Dict]): Parsed CSV data
        
    Returns:
        Dict: Mapping of column names to schema information
    """
    if not csv_data:
        return {}
    ...
    # Use multiple rows for better type inference
    ...
    for column in all_columns:
        ...
        column_values = [row.get(column) for row in sample_rows if row.get(column)]
        ...
        # Check for integers
        if all(isinstance(v, str) and v.isdigit() or isinstance(v, int) for v in column_values):
            schema[column] = {
                "dataType": "integer",
        ...
        # Check for floats
        try:
            if all(isinstance(v, (int, float)) or (isinstance(v, str) and float(v)) for v in column_values):
                schema[column] = {
                    "dataType": "float",
        ...
        for date_format in date_formats:
            try:
                # Try to parse the first value as a date
                if isinstance(column_values[0], str) and datetime.strptime(column_values[0], date_format):
                    # Check if all values match this format
                    if all(isinstance(v, str) and datetime.strptime(v, date_format) for v in column_values):
                        schema[column] = {
                            "dataType": "date",
                            "xsd_type": "xsd:date",
                            "format": date_format
        ...
        # Check for URLs
        if all(isinstance(v, str) and (v.startswith('http://') or v.startswith('https://')) for v in column_values):
            schema[column] = {
                "dataType": "url",
                "xsd_type": "xsd:anyURI"
            }
...
def generate_croissant_metadata(
    ...
    for key, dataset in datasets.items():
        ...
        # Create enhanced distribution entry with JSON-LD
        distribution = {
            "@type": ["cr:FileObject", "schema:MediaObject", "schema:DataDownload"],
            "@id": dataset['file']['path'],
            "name": dataset['file']['name'],
            "description": f"{key.capitalize()} dataset for {dataset_name}",
            "contentUrl": content_url,
            "encodingFormat": "text/csv",
            "sha256": content_hash,
            "dateModified": commit_date,
            "contentSize": f"{len(dataset['content'].encode())} bytes",
            "inLanguage": "en"
        }
    ...
    if 'train' in datasets and datasets['train']['data']:
        ...
        for column, schema_info in train_schema.items():
            field = {
            ...
            # Use enhanced schema info with XSD types
            field["dataType"] = train_schema[column]["dataType"]
            
            # Add XSD type for JSON-LD
            if "xsd_type" in train_schema[column]:
                field["@type"] = train_schema[column]["xsd_type"]
                
            # Add format if available (e.g., for dates)
            if "format" in train_schema[column]:
                field["format"] = train_schema[column]["format"]
    ...
    # Create Croissant metadata with enhanced JSON-LD
    metadata = {
        "@context": {
            "@language": "en",
        ...
        "@id": f"https://github.com/{owner}/{repo}/tree/{commit_sha}/{dataset_path}",
        "@type": ["cr:Dataset", "schema:Dataset"],
        "name": dataset_name,
        ...
        "url": f"https://github.com/{owner}/{repo}/tree/main/{dataset_path}",
        "sameAs": f"https://github.com/{owner}/{repo}/tree/{commit_sha}/{dataset_path}",
        "isAccessibleForFree": True,
        "keywords": [dataset_name, "machine learning", "computer vision", "dataset"],
        "citations": [],
        "distribution": distributions,
        "recordCount": total_samples,
        "fileCount": len(distributions),
        "temporalCoverage": commit_date.split("T")[0] if "T" in commit_date else commit_date,
        "datasetSchema": {
            "@type": "cr:TableSchema",
            "columns": data_schema
        }
...
def generate_raw_url(
    ...
    """
    Generate a raw GitHub URL for a file at a specific commit.
...
if __name__ == "__main__":
    ...
    token = os.environ.get("GITHUB_TOKEN_PADND")
    
    # Repository info
    owner = "PaperAnalyticalDeviceND"
    repo = "pad_dataset_registry"
    ...
    for dataset in datasets:
        ...
        output_file = os.path.join(output_dir, f"{dataset_name}_croissant.jsonld")
